  ...
  const validaCaja = async (newStatus) => {
    ...
    if (newStatus) {
      ...
      try {
        ...
        queryClient.invalidateQueries(['caja']);
        alert("Caja actualizada/abierta exitosamente");
  ...
  return (
    <section className="bg-[#1A1A1A] border border-white/5 p-6 rounded-2xl shadow-2xl mb-6 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 bg-black/20 px-4 py-2 rounded-2xl border border-white/5">
          ...
          <button
            ...
            className={`relative w-12 h-6 rounded-full transition-all duration-300 focus:outline-none ${
                isOpen ? 'bg-emerald-500/20 border-emerald-500/50' : 'bg-red-500/10 border-red-500/30'
            } border`}
          >
            <div
              className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 rounded-full transition-all duration-300 shadow-lg ${
                isOpen
      ...
      <button
        ...
        className={`w-full font-black py-4 rounded-xl mt-8 transition-all active:scale-95 uppercase tracking-[0.2em] text-[11px] shadow-xl ${
            isOpen
